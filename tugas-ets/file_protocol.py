...
class FileProtocol:
    ...
    def proses_string(self,string_datamasuk=''):
        try:
            ...
            if not parts:
                logging.warning("Gagal parsing string.")
                return json.dumps({"status": "ERROR", "data": "Request tidak dikenali: Gagal parsing data"})
            c_request_original = parts[0]
            c_request = c_request_original.lower().strip()
            logging.info(f"Request yang diproses setelah melakukan lower: {c_request}")
            ...
            logging.error(f"index error saat proses string: {string_datamasuk}")
            return json.dumps({"status": "ERROR", "data": f"Index error: {str(e)}"})
